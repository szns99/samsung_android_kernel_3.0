...
LIST_HEAD(s5p_psy_head);  //add by yxj for charge logo display  boot_command_line
...
static int  __init start_charge_logo_display(void)
{
	...
	list_for_each_entry(psy, &s5p_psy_head, s5p_psy_node)
	{
	...
	{
		if(system_state != SYSTEM_RESTART)   //do not enter power on charge mode when soft  reset
	    {			
			add_bootmode_charger_to_cmdline();
			//boot_mode_init("charge");
